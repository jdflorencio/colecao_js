...
var calcula_data = function(data)
	{
		...
		var mes2 = dt_dados[1];
		var ano2 = dt_dados[2];
		var dia_atual = new Date(ano1, mes1, dia1);
		var dia_registro = new Date(ano2, mes2, dia2);
		var result = dia_atual.getTime() -dia_registro.getTime();
		result /= 86400000;
...
var configLinha = function(val, objHash, objTelefone)
{
	...
	var btnConsultaSNGPC = '<button id="btnConsultaSNGPC" type="button" class="btn-outline btn-danger btn-sm"><i class="glyphicon glyphicon-refresh" aria-hidden="true"></i></button>';
	var btnObservacao = '<button id="btnObservacao" type="button" class="btn-outline btn-success btn-sm"><i class="glyphicon glyphicon-pencil" aria-hidden="true"></i></button>';	
	var btnBloqueado = '<button  id="btnBloqueado" type="button" class="btn-outline btn-secondary btn-sm" disabled><i class="glyphicon glyphicon-remove"></i></button>';
	...
	if(val[8]==1)
		{
			val[8] = '<i class="glyphicon glyphicon-exclamation-sign"></i> Sim';
		}
	else if(val[8]==2)
		{
	...
	<td >'+aviso_colum+'</td><td>'+btnMensagens+' '+btnConsultaSNGPC+' '+btnObservacao+'</td>\
							...
							if(data.statusmovimento == 1)
								{
									...
									var tab8 = document.getElementById("sngpcTable").rows[idTable+1].cells[9].innerHTML = aceito ;									
								}	
					...
					complete: function()
						{
							...
							if(data.length<1)
								{
									$("#tableHistorico").html('<h1>Vazio</h1>');
								}						
